...
def load_extension(name, sources, extra_cflags=None, extra_cuda_cflags=None, **kwargs):
    ...
    if extra_cflags is None:
        extra_cflags = ["-O3"]
        allow_openmp = os.environ.get("TORCHDRUG_ENABLE_OPENMP", "0") == "1"
        if allow_openmp and torch.backends.openmp.is_available() and os.uname().sysname != "Darwin":
...
def sparse_coo_tensor(indices, values, size):
    ...
    try:
        ...
        # Fallback path for environments where the C++ extension cannot be built
        # (e.g., macOS clang without OpenMP, constrained CI, or unsupported devices).
